...
def test_done_lifecycle(tmp_path):
    ...
    with monitor("a", total=3, path=p, min_interval=0) as m:
...
def test_failure_records_error_and_reraises(tmp_path):
    ...
    with pytest.raises(ValueError):
        with monitor("b", total=2, path=p, min_interval=0) as m:
            m.update()
...
def test_mark_patches_after_close(tmp_path):
    ...
    with monitor("d", total=1, path=p, min_interval=0) as m:
        m.update()
...
def test_read_monitors_tree(tmp_path):
    (tmp_path / "cell").mkdir()
    with monitor("sweep", 1, tmp_path / "sweep.progress.json", min_interval=0):
        pass
    with monitor("cell", 1, tmp_path / "cell" / "train.progress.json", parent="sweep", min_interval=0):
        pass
